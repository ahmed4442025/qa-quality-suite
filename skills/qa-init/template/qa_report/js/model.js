/* Read-only report data rules. Existing MODULE_* files remain unchanged. */
(function (root, factory) {
  ...
  function normalize(modules) {
    return modules.flatMap((mod, moduleIndex) => {
      ...
      return mod.tasks
        ...
        .map((t) => {
          // Engineering-only fields are deliberately discarded in the PM runtime.
          const {
            file: _file,
  ...
  function fromUrl(search, tasks) {
    ...
    if (p.get("view") === "report") state.view = "report";
    state.page = Math.min(
      100000,
